...
def inquire_balance(
    ...
    * 당일 전량매도한 잔고도 보유수량 0으로 보여질 수 있으나, 해당 보유수량 0인 잔고는 최종 D-2일 이후에는 잔고에서 사라집니다.
    ...
    if res and res.isOK():
        ...
        # .get() 메소드를 사용하여 'output1' 키가 없을 경우 비어있는 리스트([])를 기본값으로 사용합니다.
        output1_data = res.getBody().get('output1', [])
        current_data1 = pd.DataFrame(output1_data)
        ...
        # 'output2'도 동일하게 안전하게 처리합니다.
        output2_data = res.getBody().get('output2', [])
        current_data2 = pd.DataFrame(output2_data)
